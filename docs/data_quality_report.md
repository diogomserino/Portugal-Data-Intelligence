...
> **Historical snapshot (March 2026).** This report documents the validation
> exercise from the 6-pillar era and is kept for reference. The platform now
> covers **12 pillars** with the data-quality framework extended to all of
> them (v2.5.0), and current provenance lives in
> [Data Provenance](data_provenance.md). Two statements below no longer hold:
> the `is_provisional` flag is **not** populated in the committed snapshot
> (every row is final; the flag is reserved for live refreshes), and the EU
> benchmark is now generated by the pipeline rather than fetched.
