...
> rates, credit, public debt) plus the inequality and regional pillars carry
> official series fetched from open APIs (or embedded from the published
> releases), as documented below. The remaining extended pillars (housing,
> labour detail, external accounts, fiscal) and the EU benchmark are
> **modelled estimates** calibrated to published trends — see
...
**Date:** 2026-06-11 (core pillars and extended-pillar snapshots); inequality
and regional re-fetched 2026-06-12 with the official EU-SILC / NUTS2 series
**Status:** All snapshot pillars fetched successfully (the EU benchmark is
generated by the pipeline, not fetched — see below)
...
> **2026-06-12 audit (v2.5.0).** The inequality pillar was switched to the
> official Eurostat EU-SILC series (`ilc_di12`, `ilc_di11`, `ilc_li02` —
> previously a smoothed estimate that missed the 2021/2023 Gini spikes),
> regional youth unemployment was populated with the official `lfst_r_lfu3rt`
> series where published, and the dead benchmark fetch path was removed.
> See `CHANGELOG.md` (2.5.0).
